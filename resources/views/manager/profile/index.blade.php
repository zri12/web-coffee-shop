...
<div class="p-6 space-y-6" x-data="profileManager()">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-[#897561] dark:text-[#a89c92] mt-1">Manage your account settings and preferences</p>
        </div>
    </div>
                ...
                <div class="px-6 pb-6">
                    <div class="flex flex-col items-center -mt-12">
                        ...
                        <span class="mt-2 px-3 py-1 rounded-full text-xs font-semibold
                            @if(auth()->user()->role === 'manager') bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400
                            @elseif(auth()->user()->role === 'cashier') bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400
...
function profileManager() {
    return {
        ...
        async updatePassword() {
            ...
            try {
                const response = await fetch('/manager/profile/password', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
