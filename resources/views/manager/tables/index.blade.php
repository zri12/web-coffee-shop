...
<div class="p-6 space-y-6" x-data="tableManager()">
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input 
                        ...
                        class="w-full pl-10 pr-4 py-2 bg-[#f4f2f0] dark:bg-[#221910] border border-transparent focus:border-primary rounded-lg text-sm text-[#181411] dark:text-white placeholder-[#897561] dark:placeholder-[#a89c92]">
                </div>
            </div>
            
            <!-- Status Filter -->
            <div class="flex gap-2">
                <button @click="statusFilter = 'all'; filterTables()" :class="statusFilter === 'all' ? 'bg-primary text-white' : 'bg-[#f4f2f0] dark:bg-[#221910] text-[#181411] dark:text-white'" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                    All
                </button>
                <button @click="statusFilter = 'available'; filterTables()" :class="statusFilter === 'available' ? 'bg-green-600 text-white' : 'bg-[#f4f2f0] dark:bg-[#221910] text-[#181411] dark:text-white'" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                    Available
                </button>
                <button @click="statusFilter = 'occupied'; filterTables()" :class="statusFilter === 'occupied' ? 'bg-red-600 text-white' : 'bg-[#f4f2f0] dark:bg-[#221910] text-[#181411] dark:text-white'" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                    Occupied
                </button>
                <button @click="statusFilter = 'reserved'; filterTables()" :class="statusFilter === 'reserved' ? 'bg-yellow-600 text-white' : 'bg-[#f4f2f0] dark:bg-[#221910] text-[#181411] dark:text-white'" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                    Reserved
                </button>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="table-card bg-white dark:bg-[#2d2115] rounded-xl border-2 border-[#f4f2f0] dark:border-[#3e2d23] overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1" 
            ...
            <!-- QR Code -->
            <div class="p-6 flex flex-col items-center bg-gradient-to-br from-[#fdfbf7] to-[#f4f2f0] dark:from-[#221910] dark:to-[#2c241b]">
            ...
            <!-- Quick Actions -->
            <div class="p-3 bg-[#fdfbf7] dark:bg-[#221910] border-t border-[#f4f2f0] dark:border-[#3e2d23]">
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <button onclick="printQR('{{ $table->id }}')" class="flex items-center justify-center gap-1 px-3 py-2 bg-[#f4f2f0] dark:bg-[#2c241b] text-[#181411] dark:text-white rounded-lg hover:bg-[#e8e4df] dark:hover:bg-[#3e2d23] transition-colors text-sm font-medium">
                ...
                <div class="mt-2 pt-2 border-t border-[#e8e4df] dark:border-[#3e2d23]">
    ...
    <div x-show="showModal" 
        ...
        <div @click.stop 
             class="bg-white dark:bg-[#2d2115] rounded-xl shadow-2xl max-w-md w-full"
             x-transition:enter="transition ease-out duration-300"
            ...
            <form @submit.prevent="submitForm()" class="p-6 space-y-4">
                ...
                <div class="flex gap-3 pt-4">
                    <button type="button" @click="closeModal()" class="flex-1 px-4 py-2 bg-[#f4f2f0] dark:bg-[#221910] text-[#181411] dark:text-white rounded-lg hover:bg-[#e8e4df] dark:hover:bg-[#2c241b] transition-colors font-medium">
                        Cancel
                    </button>
...
function tableManager() {
    return {
        ...
        async deleteTable(id, number) {
            ...
            try {
                const response = await fetch(`/manager/tables/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    }
        ...
        async updateStatus(id, status) {
            try {
                const response = await fetch(`/manager/tables/${id}/status`, {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
