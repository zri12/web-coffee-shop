...
<div class="p-6 space-y-6" x-data="{
        ...
        <div class="px-6 py-4 border-t border-[#e6e0db] dark:border-[#3d362e] flex items-center justify-between bg-gray-50/50 dark:bg-[#1a1612]">
            ...
            <div class="flex gap-1">
                ...
                @if ($menus->hasMorePages())
                    <a href="{{ $menus->nextPageUrl() }}" class="px-3 py-1 border border-[#e6e0db] dark:border-[#3d362e] rounded bg-white dark:bg-[#1a1612] text-[#5c4d40] dark:text-[#a89c92] hover:bg-gray-50 dark:hover_bg-[#2c241b] text-sm">Next</a>
                @else
                    <span class="px-3 py-1 border border-[#e6e0db] dark_border-[#3d362e] rounded bg-gray-100 dark_bg-[#2c241b] text-[#897561] opacity-50 text-sm cursor-not-allowed">Next</span>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-red-50 dark:bg-[#1a1612] border border-red-100 dark:border-[#3d362e] p-4 rounded-xl flex items_center gap-4">
    ...
    <div x-show="showAddModal" x-transition class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showAddModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w_full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-[#1a1612] shadow-xl rounded-2xl border border-[#e6e0db] dark:border-[#3d362e]">
                @php
                ...
                <form action="{{ route('admin.menus.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xl font-bold text-[#181411] dark:text_white">Add New Menu Item</h3>
                        <button type="button" @click="showAddModal = false" class="text-[#897561] hover:text-[#181411] dark:hover:text_white transition-colors">
                    ...
                    <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                        <!-- Name -->
                        <div>
                            <label class="block text-sm font-bold text-[#181411] dark:text_white mb-2">Product Name *</label>
                            <input type="text" name="name" required class="w-full px-4 py-2 bg-gray-50 dark:bg-[#2c241b] border border-[#e6e0db] dark:border-[#3d362e] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-[#181411] dark:text_white">
                        </div>

                        <!-- Category -->
                        <div>
                            <label class="block text-sm font-bold text-[#181411] dark:text_white mb-2">Category *</label>
                            <select name="category_id" required class="w-full px-4 py-2 bg-gray-50 dark:bg-[#2c241b] border border-[#e6e0db] dark:border-[#3d362e] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-[#181411] dark:text_white">
                        ...
                        <!-- Price -->
                        <div>
                            <label class="block text-sm font-bold text-[#181411] dark:text_white mb-2">Price (Rp) *</label>
                            <input type="number" name="price" required min="0" step="1000" class="w-full px-4 py-2 bg-gray-50 dark:bg-[#2c241b] border border-[#e6e0db] dark:border-[#3d362e] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-[#181411] dark:text_white">
                        </div>

                        <!-- Description -->
                        <div>
                            <label class="block text-sm font-bold text-[#181411] dark:text_white mb-2">Description</label>
                            <textarea name="description" rows="3" class="w-full px-4 py-2 bg-gray-50 dark:bg[#2c241b] border border-[#e6e0db] dark:border-[#3d362e] rounded-lg text-sm focus:outline_none focus:ring-2 focus:ring-primary/50 text-[#181411] dark:text_white"></textarea>
                        </div>

                        <!-- Image -->
                        <div>
                            <label class="block text-sm font-bold text-[#181411] dark:text_white mb-2">Product Image</label>
                            <input type="file" name="image" accept="image/*" class="w-full px-4 py-2 bg-gray-50 dark:bg[#2c241b] border border-[#e6e0db] dark:border-[#3d362e] rounded-lg text_sm focus:outline-none focus:ring-2 focus:ring-primary/50 text[#181411] dark:text_white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-primary file:text_white hover:file:bg-primary/90">
                            <p class="text-xs text-[#897561] mt-1">Max size: 2MB. Supported formats: JPG, PNG, WEBP</p>
                        </div>

                        @php
                            $initialAddons = old('addons', [['name' => '', 'price' => '']]);
                            $initialRecipes = old('recipes', []);
                        @endphp
                        <div class="space-y-3" id="add-menu-addons">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-xs uppercase tracking-wide text-[#a89c92]">Add-ons</p>
                                    <h3 class="text-lg font-semibold text[#181411] dark:text_white">Product Add-ons</h3>
                                </div>
                                <button type="button" id="add-menu-add-addon" class="text-primary text-xs font-semibold flex items-center gap-1">
                                    <span class="material-symbols-outlined text-[18px]">add</span>
                                    Add add-on
                                </button>
                            </div>
                            <div id="add-menu-addon-container" class="space-y-2">
                        ...
                        <!-- Recipe / Ingredients -->
                        <div class="space-y-3 pt-2 border-t border[#e6e0db] dark:border[#3d362e] mt-4" id="add-menu-recipes">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-xs uppercase tracking-wide text[#a89c92]">Resep</p>
                                    <h3 class="text-lg font-semibold text[#181411] dark:text_white">Bahan per porsi</h3>
                                    <p class="text-xs text[#897561]">Stok bahan otomatis berkurang saat pesanan diproses.</p>
                                </div>
                                <button type="button" id="add-menu-add-recipe" class="text-primary text-xs font-semibold flex items-center gap-1">
                                    <span class="material-symbols-outlined text-[18px]">add</span>
                                    Tambah bahan
                                </button>
                            </div>
                            <div id="add-menu-recipe-container" class="space-y-2">
                                ...
                                <div class="grid grid-cols-12 gap-3 items-end border border[#f0e9df] dark:border[#3d362e] rounded-lg p-3 bg-gray-50/60 dark:bg[#1f1914] recipe-row">
                                    ...
                                    <div class="col-span-4">
                                        ...
                                        <input type="number" min="0.01" step="0.01" name="recipes[{{ $i }}][quantity_used]" value="{{ $recipe['quantity_used'] ?? '' }}"
                                               class="w-full px-3 py-2 border border-[#e6e0db] dark_border-[#3d362e] rounded-lg text-sm focus:outline-none focus:border-primary" placeholder="0.00">
                                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    bindAddons('#add-menu-addon-container', '#add-menu-add-addon');
    bindRecipes('#add-menu-recipe-container', '#add-menu-add-recipe');
});
</script>
@endpush
