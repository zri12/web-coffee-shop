...
@section('title', isset($menu) ? 'Edit Menu' : 'Add Menu')
...
<div class="p-6 space-y-6 max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-[#181411] dark:text-white">{{ isset($menu) ? 'Edit Menu' : 'Add Menu' }}</h1>
            <p class="text-[#897561] text-sm mt-1">{{ isset($menu) ? 'Update product details and recipe' : 'Create new menu item with recipe' }}</p>
        </div>
    ...
    <!-- Form -->
    <div class="bg-white dark:bg-[#1a1612] rounded-xl border border-[#e6e0db] dark:border-[#3d362e] shadow-sm">
        @php
            $initialRecipes = old('recipes') ?? (isset($menu) && isset($menuRecipes) ? $menuRecipes->map(function($r){
                return [
        ...
        <form action="{{ isset($menu) ? route('admin.menus.update', $menu->id) : route('admin.menus.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @if(isset($menu))
                @method('PUT')
            @endif
            ...
            <div>
                ...
                <input type="file" name="image" accept="image/*"
                       class="w-full px-4 py-3 rounded-lg border border-[#e6e0db] dark:border-[#3d362e] bg-white dark:bg-[#0f0d0b] text-[#181411] dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent text-base">
                <p class="text-xs text-[#897561] mt-2">{{ isset($menu) ? 'Leave empty to keep current image.' : '' }} Max size: 2MB. Supported formats: JPG, PNG, WEBP</p>
            </div>

            <!-- Recipe / Ingredients -->
            <div x-data="recipeForm(@json($initialRecipes))" class="space-y-3 pt-2 border-t border-[#e6e0db] dark:border-[#3d362e]">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-[#897561]">Stok bahan akan otomatis berkurang ketika pesanan masuk dapur.</p>
                    </div>
            ...
            <!-- Action Buttons -->
            <div class="flex gap-4 pt-6 border-t border-[#e6e0db] dark:border-[#3d362e]">
        ...
        @if(isset($menu))
        <!-- Recipe / Composition Section (Only for Edit) -->
        <div class="border-t border-[#e6e0db] dark:border-[#3d362e] p-6 bg-gray-50/50 dark:bg-[#0f0d0b]">
...
<!-- Add/Edit Ingredient Modal (Only for Edit Mode) -->
@if(isset($menu))
